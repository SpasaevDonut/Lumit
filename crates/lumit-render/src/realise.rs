...
use crate::draw::{AccumulationBelow, CompLayerDraw, LayerInputDraw, DrawSource};
...
impl Realiser<'_> {
    ...
    /// Render a layer's layer inputs (docs/impl/layer-input.md §2): each
    /// `LayerInputDraw` (the referenced layer's source pixels) is uploaded,
    /// linearised and resampled into the effect's working raster `(w, h)`
    /// through the shared [`crate::fxops::render_layer_input`], so the
    /// parallel `layer_inputs` handed to `run_ops` is 1:1 with the stack's
    /// layer-input ops and aligned with the layer texture. Export renders
    /// these identically (K-031).
