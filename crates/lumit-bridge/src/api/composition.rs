...
impl CompositionReference {
    ...
    /// Create a new inner composition containing `layers` and nest it as a Precomp layer in `self`.
    #[frb(sync)]
    pub fn precompose_layers(
        ...
        for layer_ref in &layers {
            let item = layer_ref.item()?;
            if let Some(idx) = outer_comp.layers.iter().position(|l| l.id == item.id) {
                target_entries.push((idx, item));
            }
        }
        ...
        target_entries.sort_by_key(|(idx, _)| *idx);
        let top_index = target_entries[0].0;

        let target_items: Vec<_> = target_entries.into_iter().map(|(_, item)| item).collect();

        let mut min_in = target_items[0].in_point.0;
        let mut max_out = target_items[0].out_point.0;
        for item in &target_items[1..] {
            if item.in_point.0 < min_in {
                min_in = item.in_point.0;
            }
            if item.out_point.0 > max_out {
                max_out = item.out_point.0;
            }
        }

        let comp_name = name.unwrap_or_else(|| {
            if target_items.len() == 1 {
                format!("{}_Precomp", target_items[0].name)
            } else {
                format!("Pre-comp 1")
            }
        });
        ...
        let mut precomp_layer = crate::edits::base_layer(
            ...
            outer_comp.duration.0,
            crate::edits::centred_transform(
                f64::from(outer_comp.width),
                f64::from(outer_comp.height),
                outer_comp.width,
                outer_comp.height,
            ),
        );
        precomp_layer.in_point = lumit_core::time::CompTime(min_in);
        precomp_layer.out_point = lumit_core::time::CompTime(max_out);
        let new_precomp_id = precomp_layer.id;
        ...
        Ok(LayerReference::new(self.project, self.id, new_precomp_id))
    }
    ...
    pub fn precompose_layer(
        ...
        self.precompose_layers(
            vec![LayerReference::new(layer.project_id, layer.comp_id, layer.layer_id)],
            name,
        )
    }
