...
/// the bake thread and outlive the frame that asked for it (K-346). The caller
...
pub struct LensFlareFx {
    ...
    /// The off-thread bake (K-346), when this engine is allowed one. `None`
    ...
    /// frame whose own bake is not ready falls back to (K-346).
...
impl LensFlareFx {
    ...
    /// - **Deferred** (the Viewer, K-346): a miss hands the bake to the bake
