...
// honest witness is the OS itself, one `GetKeyState` call away (K-334).
//
...
typedef _GetKeyStateC = Int16 Function(Int32);
typedef _GetKeyStateD = int Function(int);

_GetKeyStateD? _getKeyState;
...
/// session — the operating system agrees.
bool altActuallyHeld() {
  if (!HardwareKeyboard.instance.isAltPressed) return false;
  if (!_canAskOs) return true;
  _getKeyState ??= DynamicLibrary.open('user32.dll')
      .lookupFunction<_GetKeyStateC, _GetKeyStateD>('GetKeyState');
  return (_getKeyState!(_vkMenu) & 0x8000) != 0;
}
