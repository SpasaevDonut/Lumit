...
| `lumit-theme.svg` / `.ico` | `.lumtheme` colour themes (K-298): same chassis, three overlapping swatches in the two key gradients and the core white — colours rather than keyframes, because that is what the file carries — `THEME` kicker. The three overlap **cyclically**: blue over white, violet over blue, white over violet, which no painting order can produce, so the white swatch is painted twice — once under both, once inside the violet one. Every rim is drawn once and only where its own swatch is in front: two quarter-opacity rims stacking reads as a blot at the crossing, a rim left under the swatch in front of it reads as a line across that swatch, and a rim drawn as two arcs meeting end to end leaves a seam |
