...
- **The setting.** `Document::anti_aliasing` (`AntiAliasing::{Off,X2,X4,X8}`, default `X4`)
...
- **The capability check.** `lumit_gpu::supported_sample_count` (adapter in hand) and
  ...
  `lumit_gpu::adapter_sample_count` is the lock-free reporting path the Settings row reads.
...
- **The interface.** A **Rendering** page in the Settings window, whose section heading says
  "This project" — the one page there that is not a machine preference.
