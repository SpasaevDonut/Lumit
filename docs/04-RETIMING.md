...
- Envelope points (event time, percentage) with per-segment fade types → **RateSegments**:
  ...
  Smooth/Sharp are its own cubics, while Lumit's ease family is (piecewise-)quadratic: the
...
- Vegas's silent behaviours become visible Lumit states: media exhaustion (Vegas's "tiny V"
